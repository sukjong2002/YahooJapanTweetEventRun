...
function markExistingTweetsAsProcessed() {
  ...
  if (tweetContainers.length > 0) {
    ...
    console.log(
      `${tweetContainers.length}개의 기존 트윗이 무시 처리되었습니다.`
    );
  } else {
      ...
      if (mutation.type === "childList" && mutation.addedNodes.length > 0) {
        ...
        mutation.addedNodes.forEach((node) => {
          ...
          // 추가된 노드가 Element이고 트윗 컨테이너를 포함할 수 있는 경우
            ...
            const tweetContainers = node.querySelectorAll(
              ".Tweet_TweetContainer__aezGm"
            );
            if (tweetContainers.length > 0) {
              console.log(
...
function processTweetContainer(container) {
  ...
  console.log(
    `새 트윗 ID ${tweetId} 처리 중, 텍스트: ${cleanText.substring(0, 30)}...`
  );
  ...
  chrome.runtime.sendMessage(
    ...
    (response) => {
      if (response && response.success) {
        ...
        console.error(
          `트윗 ID ${tweetId} 처리 중 Gemini API 오류:`,
        ...
        <div class="room-info">
            ...
            <div class="player-info">
              <div class="player-count">
                <span class="player-badge">
                  <span>🧑‍🤝‍🧑${
                    tweetData.peopleRn !== undefined ? tweetData.peopleRn : "-"
                  }</span>
                  <span>명</span>
                </span>
                ${
                  tweetData.unlimited
                    ? '<span class="unlimited-badge">주회</span>'
                    : ""
                }
                ${
                  tweetData.until
                    ? `<div class="unlimited-badge">${tweetData.until}</div>`
                    : ""
                }
              </div>
            </div>
          ...
          <div class="stat-container">
            ...
            <span class="stat-value">
              ...
              ${
                tweetData.masterTotalStat
                  ? ' <span class="total-badge">내부치</span>'
                  : ""
              }
            </span>
          </div>
